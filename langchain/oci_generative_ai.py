# Modified copy of libs/community/langchain_community/chat_models/oci_generative_ai.py from the langchain repo
# https://github.com/langchain-ai/langchain
# @ 884125e
# Adds tool use for OCI GenAI Meta models.
...
class Provider(ABC):
    ...
    @abstractmethod
    def chat_stream_generation_info(
    ...
    @abstractmethod
    def oci_response_tool_calls_to_langchain(self, response: Any) -> List[ToolCall]: ...

    @abstractmethod
    def format_oci_tool_calls(
...
class CohereProvider(Provider):
    ...
    def chat_generation_info(self, response: Any) -> Dict[str, Any]:
        ...
        if response.data.chat_response.tool_calls:
            ...
            generation_info["tool_calls"] = self.format_oci_tool_calls(
    ...
    def oci_response_tool_calls_to_langchain(self, response: Any) -> List[ToolCall]:
        """Convert a OCI GenAI Cohere tool calls into langchain_core.messages.ToolCall"""
        tool_calls = response.data.chat_response.tool_calls
        return [
            ToolCall(name=tc.name, args=tc.parameters, id=uuid.uuid4().hex[:])
    ...
    def format_oci_tool_calls(
        ...
        """
        Formats a OCI GenAI Cohere API response into the tool call format used in Langchain.
        """
...
class MetaProvider(Provider):
    ...
    def __init__(self) -> None:
        ...
        self.oci_function = models.FunctionDefinition
        self.oci_function_call = models.FunctionCall
        self.oci_tool_call = models.ToolCall
    ...
    def chat_response_to_text(self, response: Any) -> str:
        if response.data.chat_response.choices[0].message.content is not None:
            return response.data.chat_response.choices[0].message.content[0].text
        else:
            return ""

    def chat_stream_to_text(self, event_data: Dict) -> str:
        if "content" in event_data["message"]:
    ...
    def chat_generation_info(self, response: Any) -> Dict[str, Any]:
        generation_info = {  # add tool_calls
        ...
        if message.role == "ASSISTANT" and message.content is not None:
            if message.content[0].text.startswith("<|python_tag|>") or message.content[
                0
            ].text.startswith("{"):
                tool_call_json = message.content[0].text.replace("<|python_tag|>", "")
                try:
    ...
    def chat_stream_generation_info(
        ...
        if "finishReason" in event_data:
            generation_info["finish_reason"] = event_data["finishReason"]

        elif "message" in event_data and "toolCalls" in event_data["message"]:
            generation_info["tool_calls"] = []
            for tool_call in event_data["message"]["toolCalls"]:

                tool_call_id = tool_call.get("id")
                if tool_call_id is not None:
                    if (
                        context["tool_call_id"] is None
                        or not tool_call_id == context["tool_call_id"]
                    ):
    ...
    def messages_to_oci_params(
        ...
        for message in messages:
            ...
            if isinstance(message, ToolMessage):
                oci_message.tool_call_id = message.tool_call_id
            if isinstance(message, AIMessage) and message.tool_calls:
    ...
    def convert_to_oci_tool(
        ...
        Convert a BaseTool instance, JSON schema dict, or BaseModel type to a OCI tool.
        """
        fn = convert_to_openai_function(tool, strict=True)
        return self.oci_function(
            name=fn["name"],
    ...
    def oci_response_tool_calls_to_langchain(self, response: Any) -> List[ToolCall]:
        ...
        return [
            ToolCall(name=tc.name, args=json.loads(tc.arguments), id=tc.id)
            for tc in tool_calls
        ]
    ...
    def with_structured_output(
        ...
        **kwargs: Any,
        ...
        if method == "function_calling":
            ...
            llm = self.bind_tools([schema], **kwargs, tool_choice=ToolChoiceRequired())
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if "tool_calls" in generation_info:
            tool_calls = self._provider.oci_response_tool_calls_to_langchain(response)
        else:
            tool_calls = []
